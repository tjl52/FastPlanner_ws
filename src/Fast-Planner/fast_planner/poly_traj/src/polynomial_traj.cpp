...
PolynomialTraj minSnapTraj(const Eigen::MatrixXd& Pos, const Eigen::Vector3d& start_vel,
  ...
  Eigen::MatrixXd poly_coeff(seg_num, 3 * 6); // 每段的多项式系数（x/y/z各6阶）
  Eigen::VectorXd Px(6 * seg_num), Py(6 * seg_num), Pz(6 * seg_num); // 所有段的x/y/z系数拼接
  ...
  for (int k = 0; k < seg_num; k++) {
    ...
    // 其他导数（如中间点的速度、加速度）默认0，后续会作为自由变量求解
  }

  /* ---------- 2. 构造映射矩阵A（多项式系数与导数边界的关系） ---------- */
  Eigen::MatrixXd Ab;
  ...
  for (int k = 0; k < seg_num; k++) {
    ...
    for (int i = 0; i < 3; i++) {
      Ab(2 * i, i) = Factorial(i); // 起点各阶导数
      for (int j = i; j < 6; j++)
        Ab(2 * i + 1, j) = Factorial(j) / Factorial(j - i) * pow(Time(k), j - i); // 终点各阶导数
    }
    A.block(k * 6, k * 6, 6, 6) = Ab; // 放到大A矩阵的对应位置
  }

  /* ---------- 3. 构造选择矩阵C'（哪些导数是固定的，哪些是自由的） ---------- */
  Eigen::MatrixXd Ct, C;

  num_f = 2 * seg_num + 4;  // 固定变量数量（起止点位置、速度、加速度等）
  num_p = 2 * seg_num - 2;  // 自由变量数量（中间点的高阶导数）
  num_d = 6 * seg_num;      // 总导数数量
  Ct = Eigen::MatrixXd::Zero(num_d, num_f + num_p);

  // 下面一大段是把固定变量和自由变量在Ct中标记出来
  Ct(0, 0) = 1;
  Ct(2, 1) = 1;
  Ct(4, 2) = 1;  // 起点位置、速度、加速度
  Ct(1, 3) = 1;
  Ct(3, 2 * seg_num + 4) = 1;
  Ct(5, 2 * seg_num + 5) = 1;
  ...
  C = Ct.transpose(); // C是Ct的转置

  // Dx1、Dy1、Dz1是所有导数的排列（固定+自由）、、选择矩阵的设计
